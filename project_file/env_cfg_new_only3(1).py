...
class EventsCfg:
    ...
    # ========================================================
    # [任务2.3:修改] 修正随机推力事件配置，移除不必要的参数
    # ========================================================
    push_robot = EventTerm(
        func=mdp.push_by_setting_velocity,  # 使用速度突变模拟冲量 / Use velocity change to simulate impulse
        mode="interval",  # 间隔模式 / Interval mode
        interval_range_s=(0.0, 0.0),  # 初始为0，由子类覆盖 / Initially 0, overridden by subclasses
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base_Link"),
            # 速度变化范围 (模拟冲量) / Velocity change range (simulating impulse)
            "velocity_range": {
                "x": (-0.5, 0.5),  # x轴方向速度变化 [m/s] / x-axis velocity change [m/s]
                "y": (-0.5, 0.5),  # y轴方向速度变化 [m/s] / y-axis velocity change [m/s]
                "yaw": (-math.pi / 12, math.pi / 12),  # 偏航角速度变化 [rad/s] / yaw angular velocity change [rad/s]
            },
            # 注意：移除了 impulse_duration 和 probability 参数，因为它们不是 push_by_setting_velocity 所需的
        },
        is_global_time=False,
        min_step_count_between_reset=0,
    )
    ...
    # ========================================================
    # [任务2.3:修改] 使用已有的随机外力函数，修正参数
    # ========================================================
    periodic_strong_disturbance = EventTerm(
        func=mdp.apply_external_force_torque_stochastic,  # 使用已有的随机外力函数 / Use existing random external force function
        mode="interval",
        interval_range_s=(8.0, 12.0),  # 8-12秒一次强干扰 / Strong disturbance every 8-12 seconds
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base_Link"),
            # 强力范围 [N] / Strong force range [N]
            "force_range": {
                "x": (-300.0, 300.0),
                "y": (-200.0, 200.0),
                "z": (-50.0, 50.0),
            ...
            "probability": 0.1,  # 每次间隔触发的概率 / Probability per interval
            # 注意：移除了 duration 参数，因为它可能不是 apply_external_force_torque_stochastic 所需的
        },
        is_global_time=False,
        min_step_count_between_reset=50,  # 至少50步后才可能触发 / Trigger only after at least 50 steps
    )
...
class RewardsCfg:
    ...
    # ========================================================
    # [任务2.3:修改] 简化抗干扰奖励项，确保函数存在且参数正确
    # ========================================================
    # 1. 抗干扰恢复奖励 - 使用速度跟踪函数
    rew_disturbance_recovery = RewTerm(
        func=mdp.track_lin_vel_xy_exp,  # 使用已有的速度跟踪函数
        weight=2.0,
        params={
            "command_name": "base_velocity",
            "std": math.sqrt(0.1)  # 更小的标准差意味着对误差更敏感
        }
    )

    # 2. 姿态抗干扰奖励 - 使用平坦朝向惩罚函数
    rew_posture_robustness = RewTerm(
        func=mdp.flat_orientation_l2,  # 使用已有的平坦朝向惩罚函数
        weight=-1.5,  # 负权重，惩罚倾斜
        # 注意：flat_orientation_l2 可能不需要额外参数
    )

    # 3. 干扰检测奖励 - 使用关节速度惩罚
    rew_disturbance_detection = RewTerm(
        func=mdp.joint_vel_l2,
        weight=-0.5,  # 惩罚关节速度变化（干扰响应）
    )

    # 4. 抗干扰能效奖励 - 使用关节功率惩罚
    pen_disturbance_energy = RewTerm(
...
class TerminationsCfg:
    ...
    # ========================================================
    # [任务2.3:修改] 简化终止条件，使用现有函数
    # ========================================================
    # 干扰后过度倾斜终止条件 - 使用已有的非法接触检测
    excessive_tilt_after_disturbance = DoneTerm(
        func=mdp.illegal_contact,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["abad_.*", "hip_.*", "knee_.*"]),
            "threshold": 20.0  # 更高的阈值，检测剧烈干扰后的异常接触
        },
    )
