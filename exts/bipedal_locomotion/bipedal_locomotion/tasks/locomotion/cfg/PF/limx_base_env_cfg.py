...
def get_gait_frequency_only(
    ...
    """只获取步态频率 - 减少观测维度，保留对速度追踪最重要的信息"""
    # 获取完整的步态命令（4维：频率、偏移、持续时间、摆动高度）
    full_gait_command = env.command_manager.get_command(command_name)
    # 只返回频率（第0维），这是对速度响应最重要的参数
    # unsqueeze(1) 保持2D形状 [num_envs, 1]
...
class ObservarionsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        gait_phase = ObsTerm(func=mdp.get_gait_phase)  # 步态相位 / Gait phase (1维)

        # 只保留步态频率，而不是完整的4维步态命令 (减少3维)
        gait_frequency = ObsTerm(
            func=get_gait_frequency_only,  # 使用自定义函数
            params={"command_name": "gait_command"},  # 步态命令名称
            noise=GaussianNoise(mean=0.0, std=0.01),  # 噪声配置
            clip=(0.5, 3.0),  # 频率合理范围 / Frequency reasonable range
            scale=0.5,  # 缩放因子 / Scaling factor
        )
...
class EventsCfg:
    ...
    # 移除抗干扰推力事件，只保留轻微扰动用于2.2任务
    push_robot = EventTerm(
        func=mdp.apply_external_force_torque_stochastic,  # 随机外力轻微扰动 / Stochastic external force mild disturbance
        mode="interval",  # 间隔模式 / Interval mode
        interval_range_s=(5.0, 10.0),  # 减少扰动频率，5-10秒一次 / Reduce disturbance frequency, once every 5-10s
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base_Link"),
            # 减小力的范围 [N] / Reduce force range [N]
            "force_range": {
                "x": (-50.0, 50.0), "y": (-50.0, 50.0), "z": (-0.0, 0.0),
            },
            # 减小力矩范围 [N⋅m] / Reduce torque range [N⋅m]
            "torque_range": {"x": (-10.0, 10.0), "y": (-10.0, 10.0), "z": (-0.0, 0.0)},
            "probability": 0.001,  # 降低发生概率 / Reduce occurrence probability
        },
        is_global_time=False,
        min_step_count_between_reset=0,
    )
...
class RewardsCfg:
    ...
    rew_base_stability = RewTerm(
        func=mdp.flat_orientation_l2,  # 使用现有的平坦朝向惩罚函数，但改为正奖励
        weight=10.0,  # 正权重，奖励姿态稳定性
        params={}
    )
    ...
    pen_base_height = RewTerm(
        ...
        weight=-10.0,  # 减少惩罚权重 / Reduce penalty weight
    )
    ...
    pen_flat_orientation = RewTerm(
        ...
        weight=-5.0  # 减少权重 / Reduce weight
    )
    pen_feet_distance = RewTerm(
        func=mdp.feet_distance,  # 足部距离惩罚 / Foot distance penalty
        weight=-50,  # 减少权重 / Reduce weight
        params={
